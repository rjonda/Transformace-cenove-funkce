...
function go() {
  if (document.getElementById("InputType-OwnData").checked) {
    ...
    if ((parseInt(document.getElementById("TrafoData").value) > 0) && (!(document.getElementById("BoilerData").value == "")) && (!(document.getElementById("GrafData").value == ""))) {
      ...
      values = document.getElementById("GrafData").value.split(";");
      console.log(document.getElementById("GrafData").value.split(";"))
...
function reset() {
  widthOfOneColumn = 100 / values.length;
...
/*    if(parseFloat(values[i]) > maxValue) {
    ...
    if (parseFloat(values[i]) > maxValue) {
      window.alert("Hodnota po transformaci je větší, než maximální hodnota!");
      document.all["grafBorder"].innerHTML = "<meta http-equiv=\"refresh\" content=\"0;url=./index.html\">"
      return false;
    }
